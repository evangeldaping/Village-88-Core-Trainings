...
<body>
    <!-- Models are PHP classes that are designed to work with information in your database. The class model contains functions to insert, update, delete and retrieve from your database.

    For the sake of this topic and succeeding tabs, we will be using this database structure below. You can also import this database into your localhost using codingdojo.sql. This database is named as "codingdojo.sql".
    ...
    Using ? - in the query method, we use what CodeIgniter calls 'query binding' to have CodeIgniter insert the values specified in the second parameter of the query method AND also ESCAPE it properly to avoid any SQL injection. ALL of your queries should be done using this query binding. For example running $this->db->query("INSERT INTO courses (title, description, created_at) VALUES (?,?,?)", array('Dojo', 'Ninja', '2013-12-25')) would run the following query: "INSERT INTO courses (title, description, created_at) VALUES ('Dojo','Ninja','2013-12-25')".
