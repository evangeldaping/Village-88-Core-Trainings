...
<body>
    ...
    Even if you are not using encrypted sessions, you must set an encryption key in your config file (/application/config/config.php) which is used to aid in preventing session data manipulation.
    ...
    Both will NOT work. Above basically retrieves the current message or messages(array) and sets it to be the new value but does not save the information back in the session. The proper way to do this is to do something like below:

    $this->session->set_userdata('message', "new message");
    $this->session->set_userdata('messages', array('new message'));
    Some of our students have spent hours trying to fix problems caused by this so make sure you're using proper syntax to update the Session data.
